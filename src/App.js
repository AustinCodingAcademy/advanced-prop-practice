...
function App(props) {
  ...
  return (
    <div>
      ...
      <div>This better say "ajax": <SuperComponent product={"ajax"} /> </div>
      {/* send in a prop called "product", give it an object with a key of "name" and a value "pepsi"*/}
      <div>This better say "pepsi": <SuperComponent product={"pepsi"} /> </div>
      {/* send in a prop called "product", give it an object with a key of "name" and a value "nike"*/}
      <div>This better say "nike": <SuperComponent product={"nike"} /> </div>


      {/* send in a prop called "names", with the value of the variable names*/}
      <div>This better say "Bob, Stand, Todd, Ted" : <SuperComponent names={["Bob", "Stand", "Todd", "Ted"]} /> </div>
      ...
      <div>This better say "Hand Sanitizer": <SuperComponent products={"Hand Snaitizer"} /> </div>
      {/* send in a prop called "names", give it the names array from App's props*/}
      <div>This better say "Robin, Lily, Barney": <SuperComponent names={["Robin", "Lily", "Barney"]} /> </div>
      {/* send in a prop called "magicNumber", give it the magicNumber from App's props*/}
      <div>This better say "99": <SuperComponent magicNumber={99} /> </div>
      {/* send in a prop called "message", give it the contact.firstName from App's props*/}
      <div>This better say "Luke": <SuperComponent message={"Luke"} /> </div>
      {/* send in a prop called "message", give it the contact.lastName from App's props*/}
      <div>This better say "Skywalker": <SuperComponent message={"Skywalker"} /> </div>
      {/* send in a prop called "message", give it the contact.occupation from App's props*/}
      <div>This better say "farmer": <SuperComponent message={"farmer"} /> </div>
      {/* send in a prop called "message", give it the contact.address from App's props*/}
      <div>This better say "300 MiddleOfNoWhere st Tatooine": <SuperComponent message={"300 MiddleOfNoWhere st Tatooine"} /> </div>
      <Parent />
    </div>
  );
}
